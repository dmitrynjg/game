  ...
  function addEnemyLevelOnLevel(xPos,yPos){
    ...
    enemy.moveAI = function(){
      ...
      this.yPos += this.grav;
      for(let i = 0; i < map.length; i++){
        if(this.xPos + this.sizeX  >= map[i].x + map[i].width && this.yPos - this.sizeY !== map[i].y){
          t = 1;
        }
        if(this.xPos - this.sizeX <= map[i].x && this.yPos - this.sizeY !== map[i].y){
          t = -1;
        }
        this.xPos += t * 0.3;
        this.changeSprite();
